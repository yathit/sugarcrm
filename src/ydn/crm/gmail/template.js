...
ydn.crm.gmail.Template.prototype.fillTemplate = function(id, opt_contact) {
  ...
  if (goog.isString(opt_contact)) {
    ...
    var query = [{
      'store': ydn.crm.sugarcrm.ModuleName.CONTACTS,
      'index': 'ydn$emails',
      'key': email,
      'limit': 1
    }, {
      'store': ydn.crm.sugarcrm.ModuleName.LEADS,
      'index': 'ydn$emails',
      'key': email,
      'limit': 1
    }];
    return sugar.send(ydn.crm.Ch.SReq.QUERY, query).addCallback(function(x) {
      ...
      if (results[0].result[0]) {
        record = new ydn.crm.sugarcrm.Record(sugar.getDomain(), ydn.crm.sugarcrm.ModuleName.CONTACTS,
            results[0].result[0]);
  ...
  var q = {
    'module': ydn.crm.sugarcrm.ModuleName.EMAIL_TEMPLATES,
    'id': id
  };
  return sugar.send(ydn.crm.Ch.SReq.GET, q).addCallback(function(x) {
    var html = x['body_html'];
    if (goog.isString(html)) {
      return this.renderTemplate(html, /** @type {ydn.crm.sugarcrm.model.Record} */ (opt_contact));
    } else {
